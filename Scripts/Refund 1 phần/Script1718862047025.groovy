...
if(diffSeconds< 86400 && status=="Thành công" && loaiGD=="Thanh toán" && PTTT=="Thẻ nội địa"&& thuonghieuthe!="MB") {
  ...
  if(textsotienmuonhoan!="" || textarea_reason!="") {
   ...
   textsotiendahoan=WebUI.getAttribute(findTestObject('Page_To yu cu hon tin/sotiendahoan1'), 'value')
	...
	System.out.println('phixuly: ' + sotiendahoan)
	...
	if((sotienmuonhoan>(giatrigd1-sotiendahoan))&& sotiendahoan==0) {
	...
	else if((sotienmuonhoan>(giatrigd1-sotiendahoan))&& sotiendahoan!=0){
	WebUI.verifyElementNotClickable(findTestObject('Page_Create Refund Request/Page_To yu cu hon tin/Full refund checked'))
	WebUI.verifyEqual(sotienmuonhoan, (giatrigd1-sotiendahoan))}
